...
<template>
  ...
  <el-form  :model="form" :rules="rules" ref="ruleForm" :inline="true" size="mini"  >
    <el-form-item :style="{width:formSet.isFilterBar?item.itemWidth||formSet.itemWidth||'':item.itemWidth||'50%',marginRight:0}"
                  :label="item.name"
                  :label-width="formSet.isFilterBar?formSet.width||item.width||'80px':formSet.width||'140px'"
                  :prop="item.key"
                  :key="item.key"
                  v-for="item in vform"
      ...
      <el-input v-model="form[item.key]"
                :type="item.type"
                autosize
                v-if="!item.render&&!formSet.isLook"
                style="width: 100%"  size="mini"
                :disabled="hidenShow(item.disabled)"
                :readonly="hidenShow(item.readonly)"
                @change="change(item)"
                :maxlength="item.maxlength"
                :minlenght="item.minlength"
                >
      </el-input>
      <el-input :value="form |itemFilter(item)"
                :type="item.type"
                autosize
                v-else-if="!item.render&&formSet.isLook"
                style="width: 100%"  size="mini"
    ...
    <el-form-item class="subBtn" v-if="formSet.submitShow&&formSet.isFilterBar" style="width: 80px;margin-left: 20px" label="" :label-width="formSet.width||'100px'">
      <el-button type="primary" @click="onSubmit">搜索</el-button>

    </el-form-item>
  </el-form>
  <div class="subBtn" v-if="formSet.submitShow&&!formSet.isFilterBar">
    ...
    export default {
      ...
      filters:{
        itemFilter(value,column){
          if(column.filter){
            return column.filter(value[column.key],value)
          }
