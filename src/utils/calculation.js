...
var float_calculator={
  ...
  add : function(s1, s2, opt) {
    ...
    if (this.isInt(s1) && this.isInt(s2)) {console.log(eval('s1' + opt + 's2'))
    ...
    var m = Math.pow(10, Math.max(this.getDigitsLength(s1), this
      .getDigitsLength(s2)));
    var n1 = this.multiply(s1, m,null);
    var n2 = this.multiply(s2, m,null);


    return (!opt || opt == '-') ? this.divide( this.toNumber(n1) - this.toNumber(n2), m,null) : this.divide( this.toNumber(n1)
      +  this.toNumber(n2), m);
  },
  /**
   * 精确的减法运算
  ...
  multiply : function(s1, s2,blNum=2) {
    ...
    ;
    return blNum?(Number(s1.replace(".", "")) * Number(s2.replace(".", ""))
      / Math.pow(10, m)).toFixed(blNum):(Number(s1.replace(".", "")) * Number(s2.replace(".", ""))
      / Math.pow(10, m));
  },
  /**
   * 精确的除法运算
  ...
  return blNum?num.toFixed(blNum):num;
